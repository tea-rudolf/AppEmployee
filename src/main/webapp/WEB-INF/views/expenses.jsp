...
<html>
	<head>
		<link rel="stylesheet" href="/resources/css/expenses.css" media="all" />
				...
				<table class="expenses">
					...
					<c:forEach items="${payPeriodForm.expenses}" var="expenses" varStatus="status">
						<tr>
							...
							<td>${expenses.date}</td>
							<td><input type="number" name="expenses[${status.index}].amount" value="${expenses.amount}"/></td>
							<td><input type="text"  name="expenses[${status.index}].comment" value="${expenses.comment}"/></td>
						</tr>
					</c:forEach>
				</table>
				<br/>
				<input type="submit" class="submit" name="submit" value="Submit the expenses" />
		</div>
		</form:form>
	</body>
</html>