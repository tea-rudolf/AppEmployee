...
public class PayPeriodService {
	...
	public void updateUserCurrentPayPeriodShiftList(String email, PayPeriod payPeriod) {
		User user = userRepository.findByEmail(email);
		user.getCurrentPayPeriod().setShiftsWorked(payPeriod.getShiftsWorked());
		userRepository.update(user);
	}

	public void updateUserCurrentPayPeriodExpenses(String email, PayPeriod payPeriod) {
		User user = userRepository.findByEmail(email);
