...
public class TimeProcessor {
	...
	@Autowired
	public TimeProcessor(PayPeriodRepository payPeriodRepository,
	...
	public PayPeriod retrieveCurrentPayPeriod()
		...
		PayPeriod currentPayPeriod = payPeriodRepository
				.findByDate(currentDate);

		if (currentPayPeriod == null) {
			throw new PayPeriodNotFoundException(
					"Cannot find project pay period containing date "
	...
	public PayPeriod retrievePreviousPayPeriod() {
		...
		return payPeriodRepository.findByDate(currentPayPeriod.getStartDate()
				.minusDays(1));
	}
	...
	public void createTimeEntry(PayPeriod payPeriod, double billableHours,
			LocalDate date, String userEmail, String taskUid, String comment)
			throws Exception {
		TimeEntry timeEntry = new TimeEntry(billableHours, date, userEmail,
				taskUid, comment);
	...
	public void editTimeEntry(String timeEntryUid, double billableHours,
			LocalDate date, String userEmail, String taskUid, String comment)
			throws Exception {
		TimeEntry timeEntry = retrieveTimeEntryByUid(timeEntryUid);
		updateTimeEntry(billableHours, date, userEmail, taskUid, comment,
				timeEntry);
	}

	private void updateTimeEntry(double billableHours, LocalDate date,
			String userEmail, String taskUid, String comment,
			TimeEntry timeEntry) throws Exception {
		timeEntry.update(billableHours, date, userEmail, taskUid, comment);
		timeEntryRepository.store(timeEntry);
	}

	private TimeEntry retrieveTimeEntryByUid(String timeEntryUid)
		...
		if (timeEntry == null) {
			throw new TimeEntryNotFoundException(
					"TimeEntry does not exist in repository");
		}
		return timeEntry;
	}
