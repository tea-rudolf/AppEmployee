...
public class TravelProcessor {
	...
	public Travel retrieveTravelByUid(String uid)
			throws TravelNotFoundException {
		Travel travel = travelRepository.findByUid(uid);

		if (travel == null) {
			throw new TravelNotFoundException(
					"Travel does not exist in repository");
		}
		return travel;
	}

	public void createTravel(double distance, String vehicle,
			LocalDate localDate, String userEmail, String comment)
			throws Exception {
		Travel travel = new Travel(distance, vehicle, localDate, userEmail,
				comment);
		travelRepository.store(travel);
	}

	public void editTravel(String travelUid, double distance, String vehicle,
			LocalDate date, String user, String comment) throws Exception {
		Travel travel = retrieveTravelByUid(travelUid);
		updateTravel(distance, vehicle, date, user, comment, travel);
	}

	public List<Travel> evaluateUserTravelsForPayPeriod(PayPeriod payPeriod,
			String userEmail) {
		...
		for (Travel travel : travelRepository.findAllTravelsByUser(userEmail)) {
			if (travel.getDate().isBefore(payPeriod.getEndDate().plusDays(1))
					&& travel.getDate().isAfter(
							payPeriod.getStartDate().minusDays(1))) {
				travels.add(travel);
			}
		}
		return travels;
	...
	private void updateTravel(double distance, String vehicle, LocalDate date,
			String user, String comment, Travel travel) throws Exception {
