...
public class DepartmentControllerTest {
	...
	public void showDepartmentsListReturnsDepartmentsListFormWhenSessionAttributeNotNull() {
		...
		String returnedForm = departmentController.showDepartmentsList(
				modelMock, sessionMock);
		assertEquals(DEPARTMENT_LIST_FORM, returnedForm);
	}

	@Test
	public void showCreateDepartmentFormReturnsCreateDepartmentFormWhenDepartmentViewModelIdValid() {
		when(departmentServiceMock.retrieveAvailableEmployeesViewModel())
				.thenReturn(departmentViewModelMock);
		String returnedForm = departmentController.showCreateDepartmentForm(
				modelMock, sessionMock);
		assertEquals(CREATE_DEPARTMENT_FORM, returnedForm);
	}

	@Test
	public void createDepartmentRedirectsToDepartmentsListWhenSuccessful()
			throws Exception {
		String returnedForm = departmentController.createDepartment(modelMock,
				departmentViewModelMock, sessionMock);
		assertEquals(DEPARTMENTS_LIST_REDIRECT, returnedForm);
	}

	@Test
	public void createDepartmentCallsCorrectServiceMethodIfSuccessful()
			throws Exception {
		departmentController.createDepartment(modelMock,
				departmentViewModelMock, sessionMock);
		verify(departmentServiceMock, times(1)).createDepartement(
				departmentViewModelMock);
	}

	@Test
	public void showEmployeesListReturnEditDepartmentFormWhenSuccessful()
			throws DepartmentNotFoundException {
		String returnedForm = departmentController.showEmployeesList(
				DEPARTMENT_NAME, modelMock, sessionMock);
	...
	public void showCreateEmployeeAccountPageReturnsUserCreationFormWhenSuccessful() {
		when(sessionMock.getAttribute(EMAIL_ATTRIBUTE)).thenReturn(EMAIL);
		String returnedForm = departmentController.showCreateEmployeeForm(
				DEPARTMENT_NAME, modelMock, userViewModelMock, sessionMock);
	...
	public void createEmployeeAccountRedirectsToEditionPageWhenRoleIsNotMissing() {
		...
		String returnedForm = departmentController.createEmployee(
				DEPARTMENT_NAME, modelMock, userViewModelMock, sessionMock);
		assertEquals(EDIT_DEPARTMENT_REDIRECT, returnedForm);
	}

	@Test
	public void createEmployeeAccountRedirectsToEditionPageWhenExceptionIsThrown()
			throws Exception {
		when(sessionMock.getAttribute(EMAIL_ATTRIBUTE)).thenReturn(EMAIL);
		when(userViewModelMock.getRole()).thenReturn(Role.EMPLOYEE.toString());
		doThrow(new Exception()).when(departmentServiceMock).createEmployee(
				EMAIL, DEPARTMENT_NAME, userViewModelMock);
		String returnedForm = departmentController.createEmployee(
				DEPARTMENT_NAME, modelMock, userViewModelMock, sessionMock);
		assertEquals(CREATE_USER_FORM, returnedForm);
	}
	...
	@Test
	public void editEmployeeReturnsEditEmployeeFormWhenExceptionIsThrown()
			throws Exception {
		when(sessionMock.getAttribute(EMAIL_ATTRIBUTE)).thenReturn(EMAIL);
		when(userViewModelMock.getRole()).thenReturn(Role.EMPLOYEE.toString());
	...
	public void showAssignEmployeeToDepartmentFormReturnsAssignEmployeeFormIfSuccessful() {
		String returnedForm = departmentController
				.showAssignEmployeeToDepartmentForm(modelMock, sessionMock);
		assertEquals(ASSIGN_EMPLOYE_FORM, returnedForm);
	}

	@Test
	public void assignEmployeeToDepartmentReturnsDepartmentsListFormIfSaveIsSuccessful()
			throws Exception {
	...
	@Test
	public void assignEmployeeToDepartmentCallsCorrectServiceMethod()
			throws Exception {
		departmentController.assignEmployeeToDepartment(
				assignationEmployeDepViewModelMock, sessionMock);
		verify(departmentServiceMock, times(1)).assignUserToDepartment(
				assignationEmployeDepViewModelMock);
	}
